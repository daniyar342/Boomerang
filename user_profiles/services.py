...
class CreateUserApiView(mixins.CreateModelMixin, generics.GenericAPIView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        return Response(
            "Код был отправлен на указанный реквизит", status=status.HTTP_201_CREATED
        )
    ...
    def check_code(code):
        try:
            ...
            if not user.is_active:
                ...
                return Response(
                    {"status": "The user is already active"},
                    status=status.HTTP_202_ACCEPTED,
                )
        except CustomUser.DoesNotExist:
            return Response("Не правильный код",status=status.HTTP_404_NOT_FOUND,)
...
class ChangePassword:
    ...
    def change_password_on_profile(request):
        ...
        if new_password != confirm_password:
            return "новый пароль не совпадает с подтвержденным"
    ...
    def change_password_on_reset(self,request):
        ...
        if new_password != confirm_password:
            return Response("новый пароль не совпадает с подтвержденным", status=status.HTTP_400_BAD_REQUEST)
